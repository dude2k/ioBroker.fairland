import type { ApiRegion, FairlandCourtyard, FairlandDataPoint, FairlandDevice } from './types';
...
export class FairlandApiClient {
    ...
    public async login(): Promise<LoginData> {
        const response = await this.fetchJson<LoginData>(`${this.baseUrl}/fyld-user-api/user/loginByPassword`, {
            method: 'POST',
    ...
    public async getCourtyards(): Promise<FairlandCourtyard[]> {
        return this.apiRequest<FairlandCourtyard[]>('/fyld-device-api/deviceGroupApi/allGroupInfo', {
    ...
    public async getAllDevicesInCourtyard(courtyardId: string): Promise<FairlandDevice[]> {
        const data = await this.apiRequest<{
            bindDeviceInfos?: FairlandDevice[];
    ...
    public async getDeviceStatus(deviceId: string): Promise<FairlandDataPoint[]> {
        return this.apiRequest<FairlandDataPoint[]>('/fyld-device-api/deviceDataPointApi/deviceDataPointInfo', {
            deviceId,
        });
    }
    ...
    private async apiRequest<T>(path: string, payload: Record<string, unknown>, retryOnAuth = true): Promise<T> {
        if (!this.token) {
