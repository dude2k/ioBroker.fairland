...
class FairlandAdapter extends utils.Adapter {
  ...
  private async onStateChange(id: string, state: ioBroker.State | null | undefined): Promise<void> {
    ...
    try {
      await this.handleWritableState(localId, mapping, state.val as StateValue);
    } catch (error) {
  ...
  private async ensureHeatPumpStates(
  ...
  private async ensureWaterPumpStates(
    device: FairlandDevice,
    dpMap: Map<string, FairlandDataPoint>,
  ): Promise<void> {
    const deviceBase = this.deviceBase(device);
  ...
  private async updateHeatPumpStates(
    device: FairlandDevice,
    dpMap: Map<string, FairlandDataPoint>,
  ): Promise<void> {
    const deviceBase = this.deviceBase(device);
  ...
  private async updateWaterPumpStates(
    device: FairlandDevice,
    dpMap: Map<string, FairlandDataPoint>,
  ): Promise<void> {
    const deviceBase = this.deviceBase(device);
  ...
  private async updateRawStates(
    ...
    for (const dp of dpMap.values()) {
      ...
      await this.setStateAsync(
        `${this.deviceBase(device)}.raw.dp_${sanitizeObjectId(dp.dpId)}`,
        {
  ...
  private async handleWritableState(
    ...
    const rawValue = mapping.kind === "heatTargetTemperature"
      ? scaleWrite(coerced, mapping.scale ?? 0)
      : coerced;
  ...
  private async writeHeatHvacMode(
    localId: string,
    mapping: WritableStateMapping,
    value: StateValue,
  ): Promise<void> {
